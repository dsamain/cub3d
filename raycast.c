...
/*   Updated: 2022/07/01 22:37:35 by dsamain          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ray_hit(t_data *data, t_ray *ray, int x)
{
	if (!ray->side)
		ray->wall_dist += ray->side_dist_x - ray->dx;
	else
		ray->wall_dist += ray->side_dist_y - ray->dy;
	ray->line_height = (double)data->height / (ray->wall_dist / 1.1);
	if (is_out(data, ray->map_x, ray->map_y))
		return (1);
	if (is_portal(data, ray) != -1 && data->portal[0].ori
		!= -1 && data->portal[1].ori != -1)
		manage_portal(data, ray, is_portal(data, ray), x);
	return (0);
}

int	ray_cast(t_data *data, t_ray *ray, int x)
{
	set_side_dist(ray);
	while (!ray->hit)
	{
